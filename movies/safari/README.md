...
Every file in `roc/` says so in its header, including `Text.roc` -- which came
from the compiler's own CCE tables rather than from safari's chapters, and is
frozen too. Nothing in this directory is regenerated by anything.
