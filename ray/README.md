...
| a frame as shapes a platform can fill | `safari/roc/Shapes.roc`: convex polygons, ear-clipped triangles, discs, glows, rectangles | any |
...
1. **Shapes** (the default): roc-ray fills every polygon, triangle and disc
   ...
   into triangles.
2. **Pixels**: Raster paints the whole frame in Roc, as the canvas would, and
   roc-ray shows it as one texture.

Headless on the build box (Linux, no window), a frame takes about 17 ms with
Shapes and 34 ms with Pixels. Gradients are drawn flat with Shapes for now, and
edges are not anti-aliased.

Keys: SPACE pauses and resumes, UP and DOWN step, J rides to the next segment,
D shows the frame rate, R switches painter, ESCAPE quits.
...
builds `hello` and `safari`, runs both headless, and uploads them.
