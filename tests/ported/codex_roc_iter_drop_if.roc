...
#
#   from      https://github.com/damiant3/Cobblestone/blob/master/codex/test/roc-iter-drop-if.codex
...
# RocIterDropIf -- emitted from Codex by rocemit (rust-codex-compiler). Do not edit.
...
iter_drop_if : Iter_(a), (a -> Bool) -> Iter_(a)
iter_drop_if = |it, pred| Iter_.{ next: ({
	dev__1 = it
	dev__2 = pred
...
lam_0 = |it, pred, _ignored| (match (it.next)(0) {
	One(item, rest) => (if pred(item) { (iter_drop_if(rest, pred).next)(0) } else { One(item, iter_drop_if(rest, pred)) })
	Done => Done
})
...
main! = |_args| {
	({
		dropped = iter_drop_if(range_to(1, 4), lam_2)
		(match (dropped.next)(0) {
