...
#
#   from      https://github.com/damiant3/Cobblestone/blob/master/codex/test/units-foreword.codex
...
main! = |_args| {
	({
		...
		power = horsepower(1)
		({
			line!(Text.printed(List.concat(List.concat([22, 25, 21, 69, 2], Text.show_int(duration_to_Millisecond(dur))), [26, 19])))
			line!(Text.printed(List.concat(List.concat([23, 13, 18, 69, 2], Text.show_int(length_to_Kilometer(total_len))), [34, 26])))
			line!(Text.printed(List.concat(List.concat([15, 19, 14, 21, 16, 69, 2], Text.show_int(astroDistance_to_AstroKm(star_dist))), [34, 26])))
			line!(Text.printed(List.concat(List.concat([22, 15, 14, 15, 69, 2], Text.show_int(dataSize_to_Megabyte(data))), [52, 58])))
			line!(Text.printed(List.concat([14, 13, 26, 31, 73, 34, 69, 2], Text.show_int(bk))))
			line!(Text.printed(List.concat([14, 13, 26, 31, 73, 28, 69, 2], Text.show_int(bf))))
			line!(Text.printed(List.concat([14, 13, 26, 31, 73, 24, 69, 2], Text.show_int(fc))))
			line!(Text.printed(List.concat([15, 18, 29, 23, 13, 73, 15, 21, 24, 26, 17, 18, 69, 2], Text.show_int(angle_to_Arcminute(angle)))))
			line!(Text.printed(List.concat([19, 31, 13, 13, 22, 73, 26, 81, 19, 69, 2], Text.show_int(speed_to_MeterPerSec(speed)))))
			line!(Text.printed(List.concat([13, 18, 13, 21, 29, 30, 73, 35, 69, 2], Text.show_int(energy_to_Joule(energy)))))
			line!(Text.printed(List.concat([28, 16, 21, 24, 13, 73, 26, 18, 69, 2], Text.show_int(force_to_Millinewton(force_1)))))
			line!(Text.printed(List.concat([31, 16, 27, 13, 21, 73, 27, 69, 2], Text.show_int(power_to_Watt(power)))))
		})
	})
	Ok({})
}
