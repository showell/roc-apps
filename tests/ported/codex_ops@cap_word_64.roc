...
main! = |args| {
	...
	(_mem12, mem__6) = ({
		...
		({
			...
			_ = line!(Text.printed(List.concat([37, 73, 32, 17, 14, 6, 4, 2, 69, 2], Text.show_int(mem__1))))
			(mem8, mem__2) = Mem.load!(mem7, a, 8, 8)
			_ = line!(Text.printed(List.concat([37, 73, 32, 17, 14, 7, 3, 2, 69, 2], Text.show_int(mem__2))))
			(mem9, mem__3) = Mem.load!(mem8, a, 16, 8)
			_ = line!(Text.printed(List.concat([37, 73, 19, 26, 15, 23, 23, 2, 69, 2], Text.show_int(mem__3))))
			(mem10, mem__4) = Mem.load!(mem9, a, 24, 4)
			_ = line!(Text.printed(List.concat([22, 73, 19, 26, 15, 23, 23, 2, 69, 2], Text.show_int(mem__4))))
			({
				(mem11, mem__5) = Mem.load!(mem10, a, 32, 4)
				(mem11, line!(Text.printed(List.concat([22, 73, 32, 17, 14, 7, 3, 2, 69, 2], Text.show_int(mem__5)))))
			})
		})
	})
	mem__6
