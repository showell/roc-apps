...
#
#   from      https://github.com/damiant3/Cobblestone/blob/master/codex/test/osc-noise.codex
...
ont_bank : Oscillator.OscBank
ont_bank = Oscillator.osc_add(Oscillator.osc_bank_new(ont_rate), ont_freq, 1000, Oscillator.osc_type_noise, 0)
...
ont_seen_before : List(I64), I64, I64 -> Bool
ont_seen_before = |xs, i, j| (if (j >= i) { False } else { (if ((List.get(xs, I64.to_u64_wrap(j)) ?? crash("list-at out of range")) == (List.get(xs, I64.to_u64_wrap(i)) ?? crash("list-at out of range"))) { True } else { ont_seen_before(xs, i, (j + 1)) }) })

ont_distinct : List(I64), I64, I64, I64 -> I64
ont_distinct = |xs, i, n, acc| (if (i >= n) { acc } else { ont_distinct(xs, (i + 1), n, (acc + (if ont_seen_before(xs, i, 0) { 0 } else { 1 }))) })
...
main! = |_args| {
	...
	line!(Text.printed(List.concat([19, 15, 26, 31, 23, 13, 19, 69, 2, 2, 2, 2, 2, 2, 2], Text.show_int(ont_samples))))
	line!(Text.printed(List.concat([31, 13, 21, 17, 16, 22, 69, 2, 2, 2, 2, 2, 2, 2, 2], Text.show_int(ont_period))))
	line!(Text.printed(List.concat(List.concat(List.concat([21, 13, 31, 13, 15, 14, 19, 69, 2, 2, 2, 2, 2, 2, 2], Text.show_int(reps)), [2, 16, 28, 2]), Text.show_int((ont_samples - ont_period)))))
	line!(Text.printed(List.concat([22, 17, 19, 14, 17, 18, 24, 14, 69, 2, 2, 2, 2, 2, 2], Text.show_int(dist))))
	line!(Text.printed(List.concat([18, 16, 14, 2, 31, 13, 21, 17, 16, 22, 17, 24, 69, 2, 2], (if (reps < (ont_samples - ont_period)) { [40, 21, 25, 13] } else { [54, 15, 23, 19, 13] }))))
	line!(Text.printed(List.concat([15, 32, 16, 33, 13, 2, 31, 13, 21, 17, 16, 22, 69, 2, 2], (if (dist > ont_period) { [40, 21, 25, 13] } else { [54, 15, 23, 19, 13] }))))
	Ok({})
}
