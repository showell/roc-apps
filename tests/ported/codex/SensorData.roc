...
SensorData :: [].{
	SensorKind : [Temperature, Humidity, Barometer, Light, Accelerometer, Gyroscope, Magnetometer, Gps, Battery, Custom(Str)]
	SensorValue : [IntValue(I64), FixedValue(I64, I64), BoolValue(Bool), Vec3Value(I64, I64, I64)]
	SensorReading : { kind : SensorData.SensorKind, value : SensorData.SensorValue, timestamp : I64, device_id : Str }
	...
	sensor_kind_name : SensorData.SensorKind -> Str
	...
	sensor_value_to_text : SensorData.SensorValue -> Str
	sensor_value_to_text = |v| (match v {
		IntValue(n) => I64.to_str(n)
		FixedValue(whole, frac) => Str.concat(Str.concat(I64.to_str(whole), "."), I64.to_str(frac))
		BoolValue(b) => (if b { "true" } else { "false" })
		Vec3Value(x, y, z) => Str.concat(Str.concat(Str.concat(Str.concat(I64.to_str(x), ","), I64.to_str(y)), ","), I64.to_str(z))
	})

	make_reading : SensorData.SensorKind, SensorData.SensorValue, I64, Str -> SensorData.SensorReading
	make_reading = |kind, value, ts, dev| { kind: kind, value: value, timestamp: ts, device_id: dev }

	format_reading : SensorData.SensorReading -> Str
	format_reading = |r| Str.concat(Str.concat(Str.concat(Str.concat(Str.concat(Str.concat(r.device_id, "/"), sensor_kind_name(r.kind)), "="), sensor_value_to_text(r.value)), "@"), I64.to_str(r.timestamp))
