...
Cbor :: [].{
	CborValue := [CborUint(I64), CborNint(I64), CborBytes(List(I64)), CborText(List(U8)), CborArray(List(Cbor.CborValue)), CborMap(List(Cbor.CborMapEntry)), CborBool(Bool), CborNull, CborTag(I64, Cbor.CborValue)].{
	...
	cbor_encode_text_value : List(U8) -> List(I64)
	...
	cbor_text_to_bytes : List(U8), I64, I64, List(I64) -> List(I64)
	...
	cbor_encode_text : List(U8) -> List(I64)
	...
	cbor_bytes_to_text : List(I64), I64, I64, List(U8) -> List(U8)
	cbor_bytes_to_text = |bs, i, len, acc| (if (i >= len) { acc } else { cbor_bytes_to_text(bs, (i + 1), len, List.concat(acc, Text.char_to_text((List.get(bs, I64.to_u64_wrap(i)) ?? crash("list-at out of range"))))) })
