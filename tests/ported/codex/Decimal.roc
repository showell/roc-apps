...
Decimal :: [].{
	...
	dec_new = |mantissa, scale| Decimal.Decimal.{ dec_mantissa: mantissa, dec_scale: scale }

	dec_zero : Decimal.Decimal
	dec_zero = Decimal.Decimal.{ dec_mantissa: 0, dec_scale: 2 }

	dec_one : I64 -> Decimal.Decimal
	dec_one = |scale| Decimal.Decimal.{ dec_mantissa: dec_pow10(scale), dec_scale: scale }

	dec_from_int : I64, I64 -> Decimal.Decimal
	dec_from_int = |n, scale| Decimal.Decimal.{ dec_mantissa: (n * dec_pow10(scale)), dec_scale: scale }

	dec_from_parts : I64, I64, I64 -> Decimal.Decimal
	dec_from_parts = |whole, frac, scale| Decimal.Decimal.{ dec_mantissa: ((whole * dec_pow10(scale)) + frac), dec_scale: scale }
	...
	dec_add = |a, b| ({
		...
		mb : I64
		mb = dec_promote(b, s)
		Decimal.Decimal.{ dec_mantissa: (ma + mb), dec_scale: s }
	})
	...
	dec_mul = |a, b| ({
		...
		Decimal.Decimal.{ dec_mantissa: (if (excess > dec_max_scale_digits) { 0 } else { I64.div_trunc_by(raw, dec_pow10(excess)) }), dec_scale: kept }
	})

	dec_div : Decimal.Decimal, Decimal.Decimal -> Decimal.Decimal
	dec_div = |a, b| (if (b.dec_mantissa == 0) { Decimal.Decimal.{ dec_mantissa: 0, dec_scale: a.dec_scale } } else { ({
		s : I64
		s = dec_max_scale(a, b)
		ma : I64
		ma = dec_promote(a, s)
		mb : I64
		mb = dec_promote(b, s)
		Decimal.Decimal.{ dec_mantissa: I64.div_trunc_by((ma * dec_pow10(s)), mb), dec_scale: s }
	}) })

	dec_negate : Decimal.Decimal -> Decimal.Decimal
	dec_negate = |d| Decimal.Decimal.{ dec_mantissa: (0 - d.dec_mantissa), dec_scale: d.dec_scale }

	dec_abs : Decimal.Decimal -> Decimal.Decimal
	dec_abs = |d| Decimal.Decimal.{ dec_mantissa: (if (d.dec_mantissa < 0) { (0 - d.dec_mantissa) } else { d.dec_mantissa }), dec_scale: d.dec_scale }
	...
	dec_round = |d, new_scale| (if (new_scale >= d.dec_scale) { d } else { ({
		...
		Decimal.Decimal.{ dec_mantissa: I64.div_trunc_by(adjusted, factor), dec_scale: new_scale }
	}) })
	...
	dec_truncate = |d, new_scale| (if (new_scale >= d.dec_scale) { d } else { ({
		factor : I64
		factor = dec_pow10((d.dec_scale - new_scale))
		Decimal.Decimal.{ dec_mantissa: I64.div_trunc_by(d.dec_mantissa, factor), dec_scale: new_scale }
	}) })
	...
	dec_ceiling = |d| ({
		...
		(if (d.dec_mantissa > (floored.dec_mantissa * dec_pow10(d.dec_scale))) { Decimal.Decimal.{ dec_mantissa: (floored.dec_mantissa + 1), dec_scale: 0 } } else { floored })
	})
	...
	dec_from_text = |s, default_scale| ({
		...
		(if (dot < 0) { Decimal.Decimal.{ dec_mantissa: (CceText.to_integer(s) * dec_pow10(default_scale)), dec_scale: default_scale } } else { ({
			whole_str : CceText
			...
			Decimal.Decimal.{ dec_mantissa: (sign * (((if (whole < 0) { (0 - whole) } else { whole }) * dec_pow10(scale)) + frac)), dec_scale: scale }
		}) })
	})
