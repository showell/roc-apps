...
CircularBuffer :: [].{
	...
	circbuf_new = |cap| { cb_data: ListUtils.list_zeros(cap), cb_head: 0, cb_tail: 0, cb_count: 0, cb_capacity: cap }
	...
	circbuf_push_back = |buf, val| (if (buf.cb_count >= buf.cb_capacity) { ({
		...
		{ cb_data: new_data, cb_head: MathLib.math_mod((buf.cb_head + 1), buf.cb_capacity), cb_tail: MathLib.math_mod((buf.cb_tail + 1), buf.cb_capacity), cb_count: buf.cb_count, cb_capacity: buf.cb_capacity }
	}) } else { ({
		new_data = (List.set(buf.cb_data, I64.to_u64_wrap(buf.cb_tail), val) ?? crash("list-set-at past the end"))
		{ cb_data: new_data, cb_head: buf.cb_head, cb_tail: MathLib.math_mod((buf.cb_tail + 1), buf.cb_capacity), cb_count: (buf.cb_count + 1), cb_capacity: buf.cb_capacity }
	}) })
	...
	circbuf_push_front = |buf, val| ({
		...
		(if (buf.cb_count >= buf.cb_capacity) { ({
			...
			{ cb_data: (List.set(buf.cb_data, I64.to_u64_wrap(new_head), val) ?? crash("list-set-at past the end")), cb_head: new_head, cb_tail: new_tail, cb_count: buf.cb_count, cb_capacity: buf.cb_capacity }
		}) } else { { cb_data: (List.set(buf.cb_data, I64.to_u64_wrap(new_head), val) ?? crash("list-set-at past the end")), cb_head: new_head, cb_tail: buf.cb_tail, cb_count: (buf.cb_count + 1), cb_capacity: buf.cb_capacity } })
	})

	circbuf_pop_front : CircularBuffer.CircBuf -> CircularBuffer.CircBuf
	circbuf_pop_front = |buf| (if (buf.cb_count == 0) { buf } else { { cb_data: buf.cb_data, cb_head: MathLib.math_mod((buf.cb_head + 1), buf.cb_capacity), cb_tail: buf.cb_tail, cb_count: (buf.cb_count - 1), cb_capacity: buf.cb_capacity } })

	circbuf_pop_back : CircularBuffer.CircBuf -> CircularBuffer.CircBuf
	circbuf_pop_back = |buf| (if (buf.cb_count == 0) { buf } else { { cb_data: buf.cb_data, cb_head: buf.cb_head, cb_tail: MathLib.math_mod(((buf.cb_tail - 1) + buf.cb_capacity), buf.cb_capacity), cb_count: (buf.cb_count - 1), cb_capacity: buf.cb_capacity } })
