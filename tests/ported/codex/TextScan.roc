...
TextScan :: [].{
	...
	text_match_at_loop : List(U8), List(U8), I64, I64, I64 -> Bool
	...
	text_repeat : List(U8), I64 -> List(U8)
	text_repeat = |s, n| text_repeat_loop(s, n, 0, [])

	text_repeat_loop : List(U8), I64, I64, List(U8) -> List(U8)
	text_repeat_loop = |s, n, i, acc| (if (i >= n) { acc } else { text_repeat_loop(s, n, (i + 1), List.concat(acc, s)) })

	text_pad_left_char : List(U8), I64, I64 -> List(U8)
	text_pad_left_char = |s, width, pad_char| ({
		slen = Text.len(s)
		(if (slen >= width) { s } else { List.concat(text_repeat(Text.char_to_text(pad_char), (width - slen)), s) })
	})
}
