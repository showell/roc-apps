...
Tensor :: [].{
	...
	tensor_new = |rows, cols| Tensor.Tensor.{ data: tensor_zeros((rows * cols), 0, []), rows: rows, cols: cols }

	tensor_from : I64, I64, List(I64) -> Tensor.Tensor
	tensor_from = |rows, cols, data| Tensor.Tensor.{ data: data, rows: rows, cols: cols }
	...
	tensor_vector = |data| Tensor.Tensor.{ data: data, rows: U64.to_i64_wrap(List.len(data)), cols: 1 }
	...
	tensor_set = |t, row, col, val| Tensor.Tensor.{ data: (List.set(t.data, I64.to_u64_wrap(((row * t.cols) + col)), val) ?? crash("list-set-at past the end")), rows: t.rows, cols: t.cols }
	...
	tensor_add = |a, b| Tensor.Tensor.{ data: tensor_ewise_add(a.data, b.data, 0, (a.rows * a.cols), []), rows: a.rows, cols: a.cols }
	...
	tensor_scale = |t, s| Tensor.Tensor.{ data: tensor_scale_loop(t.data, s, 0, (t.rows * t.cols), []), rows: t.rows, cols: t.cols }
	...
	tensor_ewise_mul = |a, b| Tensor.Tensor.{ data: tensor_hadamard(a.data, b.data, 0, (a.rows * a.cols), []), rows: a.rows, cols: a.cols }
	...
	tensor_trans_build = |t, r, c, acc| (if (r >= t.cols) { Tensor.Tensor.{ data: acc, rows: t.cols, cols: t.rows } } else { (if (c >= t.rows) { tensor_trans_build(t, (r + 1), 0, acc) } else { tensor_trans_build(t, r, (c + 1), List.append(acc, tensor_get(t, c, r))) }) })
