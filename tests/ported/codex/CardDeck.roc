...
CardDeck :: [].{
	...
	rank_short : I64 -> Str
	rank_short = |r| (if (r == 0) { "A" } else { (if (r == 9) { "T" } else { (if (r == 10) { "J" } else { (if (r == 11) { "Q" } else { (if (r == 12) { "K" } else { I64.to_str((r + 1)) }) }) }) }) })

	format_card : I64 -> Str
	format_card = |card| Str.concat(rank_short(card_rank(card)), suit_symbol(card_suit(card)))

	format_card_long : I64 -> Str
	format_card_long = |card| Str.concat(Str.concat(rank_name(card_rank(card)), " of "), suit_name(card_suit(card)))
	...
	format_hand : List(I64) -> Str
	format_hand = |cards| format_hand_loop(cards, 0, U64.to_i64_wrap(List.len(cards)), "")

	format_hand_loop : List(I64), I64, I64, Str -> Str
	format_hand_loop = |cards, i, len, acc| (if (i >= len) { acc } else { ({
		sep = (if (i == 0) { "" } else { " " })
		format_hand_loop(cards, (i + 1), len, Str.concat(Str.concat(acc, sep), format_card((List.get(cards, I64.to_u64_wrap(i)) ?? crash("list-at out of range")))))
	}) })
