...
one() {
    ...
    if grep -qx 'import Machine' "$d"/*.roc; then
        cp "$MACHINE"/{Machine,MachineCaps,MachineE1000,MachineHpet,MachineMem,MachinePci,MachinePorts,MachineDisk}.roc "$d/"
        ...
        stamp="$stamp machine $( { cat "$MACHINE"/{Machine,MachineCaps,MachineE1000,MachineHpet,MachineMem,MachinePci,MachinePorts,MachineDisk}.roc "$d/MachineMedia.roc"; echo "${vmargs[*]}"; stat -L -c '%s %Y' "$d"/drive*.disk "$d"/keys.txt 2>/dev/null; } | md5sum | cut -c1-16)"
    fi
