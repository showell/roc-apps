...
diverges() {
    case "$1" in
        ...
        cost@accumulator-corpus|ops@list-growth|heap-scrub|engine-culling-cost|engine-render-heap|net-recv-heap|net-ack-leak) echo "measures Codex's bump pointer with __heap-save; Roc counts references and has none, so every measurement reads zero" ;;
...
one() {
    ...
    if grep -qx 'import Machine' "$d"/*.roc; then
        cp "$MACHINE"/{Machine,MachineApic,MachineCaps,MachineE1000,MachineGpu,MachineHpet,MachineIde,MachineMem,MachineNat,MachineNe2k,MachinePci,MachinePorts,MachineScreen,MachineWire,MachineDisk}.roc "$d/"
