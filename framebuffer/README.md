...
- **Keys and the mouse** reach a running program through memory the page shares
  ...
  isolated, which the site's Caddy asks for, and a secure context: from
...
| `apps/globe/GlobeDemo.codex` | Cobblestone's globe: an icosphere wrapped in the 2048 x 1024 earth image it loads from disk, shaded by codex-vm's globe shader, turning; with no image it paints a planet in Codex. Its `gtris` calls `gtris-next`, which only calls `gtris` back; rocemit writes that call as `gtris` calling itself, which Roc runs as a loop, so the page's stack holds |
