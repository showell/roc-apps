...
mkdir -p "$NEXT" "$HERE/platform/targets/x64musl"
# **THE HOSTS ARE BUILT FAST, NOT DEBUG**: every read and write of the
# program's memory is a call into one. The native host, verify.sh's, links with
# musl's C runtime, copied from the roc checkout's fx test platform.
(cd "$HERE" && "$ZIG" build -Drelease -Droc="$ROC_CHECKOUT" --cache-dir "$HOME/build/roc-apps/zig-cache" --global-cache-dir "$HOME/build/zig-global")
