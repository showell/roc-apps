// The framebuffer platform's host: platform/host.zig compiled to a
// wasm32-freestanding object against roc's own `builtins` and `host_alloc`
// modules, taken from the roc checkout (-Droc=<path>, default
// ~/showell_repos/roc). The module graph is the one roc's build.zig gives the
// same host in its tests (test/wasm and test/provided-callable-host);
// options.zig stands in for roc's generated build options. The object lands
// where the platform header expects it.
