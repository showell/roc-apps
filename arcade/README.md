...
| `lib/` | a package: `Game`, `Keys`, `Random`, `Shapes`, `Brush`, `Font`, `Camera`, `View` |
| `web/` | the page's end: the wasm platform and host, `shapewire.js` (a frame, **painted**), `canvas_app_runner.js` (the clock, the input, the speaker), `page_check.mjs`, `camera_check.mjs` |
| `native/` | roc-ray's end: `GameRunner.roc` |

## Nobody writes the frame reader

**The platform declares what a frame IS** -- `frame : Box(model) -> List(Frame.Shape)`
-- so the compiler's own type table carries the vocabulary, and `build.sh` runs
`roc glue` with `glue/JsGlue.roc` to emit `roc_glue.js` beside the page. That
file is generated by the compiler that built the wasm, never checked in and
never edited. `shapewire.js` paints what it answers and nothing else.

What this deleted: `lib/ShapeWire.roc`, the 158-line Roc packer, and 96 lines
of hand-written decoder in `shapewire.js`. Those two were one format written
twice with nothing checking that the halves agreed, which is the failure this
repository kept hitting.

**The app did not change.** `Shapes.Shape` is a structural union, so it unifies
with the platform's `Frame.Shape` by shape rather than by name; `GameApp`
stopped calling `pack` and that was all.

    snake, one frame, 432 shapes       Roc      read     total
    packed by hand, read by hand      1.192    1.737    2.929 ms
    typed, read by generated code     0.336    0.205    0.541 ms

**The host frees nothing.** Releasing a `List(Shape)` means walking each
element's inner lists, which needs the tag union's layout -- and the host has
no business knowing that. So the platform requires a `release` the app answers
with `|_frame| {}`, the host hands the last frame back, and Roc drops it with
the layout it already has. Zero growth over 4,000 frames, and not one line of
layout knowledge in Zig.

**One call for a frame.** `computeFrame()` is the effect and answers the
address of the Roc list. It used to be two exports, `computeFrame()` then
`frameAt()`, and that arrangement caught two people who each wrote
`decode(memory, frameAt(), computeFrame())` -- which JavaScript evaluates left
to right. The same bug came back a third time as
`read(new DataView(memory.buffer), computeFrame())`: asking for a frame can
grow wasm memory, which DETACHES every view over the old buffer. **The hazard
is not two calls, it is any expression that captures a view of wasm memory
before an effect that can move it.** The view is built on its own line, after.
...
`shapewire.js` four view marks and checks the matrix it builds against the map
