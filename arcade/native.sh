...
# <game>_native.roc names as ../../roc-ray -- a sibling of roc-apps. That is a
...
OPT="${OPT:-dev}"
name="${1:?usage: arcade/native.sh <game>}"
app="$HERE/${name}_native.roc"
[ -f "$app" ] || { echo "no app at $app"; exit 2; }
ray="$HERE/../../roc-ray"
[ -d "$ray" ] || { echo "no roc-ray beside roc-apps at $ray; ${name}_native.roc names that path"; exit 2; }
case "$TARGET" in
...
t0=$(date +%s)
(cd "$HERE" && "$ROC" build "${name}_native.roc" --target="$TARGET" --opt="$OPT" --output="$exe") > "$log" 2>&1 || true
secs=$(( $(date +%s) - t0 ))
