...
# An evaluator that walks bytes and recurses, so it is not inlined away.
eval : M, List(U8), U64, F64 -> R
eval = |m, b, i, acc|
	if i >= List.len(b) {
		{ m: m, v: acc, at: i }
	} else {
		eval(m, b, i + 1, acc + U8.to_f64(List.get(b, i) ?? 0))
	}
...
spin = |m, b, i, n|
	if i >= n {
		...
		{ m: m1, v, at } = eval(m, b, 0, 0.0)
		spin({ ..m1, num: List.set(m1.num, 7, v) ?? crash("oob"), pc: at }, b, i + 1, n)
	}
main! = |args| {
