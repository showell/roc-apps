...
Renderer3D :: [].{
	...
	r3d_raster_fan! = |mem, st, poly, i, color, half_w, half_h| (if ((i + 1) >= U64.to_i64_wrap(List.len(poly))) { (mem, 0) } else { ({
		...
		r3d_raster_fan!(mem1, st, poly, (i + 1), color, half_w, half_h)
	}) })
	...
	r3d_sh_taps! = |mem, ctx, ld, tx, ty, bias, oy, ox, acc| (if (oy > r3d_pcf_radius) { (mem, acc) } else { (if (ox > r3d_pcf_radius) { r3d_sh_taps!(mem, ctx, ld, tx, ty, bias, (oy + 1), (0 - r3d_pcf_radius), acc) } else { ({
		(mem1, h) = r3d_sh_tap!(mem, ctx, ld, (tx + ox), (ty + oy), bias)
		r3d_sh_taps!(mem1, ctx, ld, tx, ty, bias, oy, (ox + 1), (acc + h))
	}) }) })
	...
	r3d_raster_fan_sh! : Mem.Mem, Renderer3D.R3dTriState, List(Renderer3D.ClipVert), I64, Renderer3D.R3dShadeCtx, I64, I64 => (Mem.Mem, I64)
	r3d_raster_fan_sh! = |mem, st, poly, i, ctx, half_w, half_h| (if ((i + 1) >= U64.to_i64_wrap(List.len(poly))) { (mem, 0) } else { ({
		p0 = r3d_project_clip((List.get(poly, I64.to_u64_wrap(0)) ?? crash("list-at out of range")), half_w, half_h)
		pa = r3d_project_clip((List.get(poly, I64.to_u64_wrap(i)) ?? crash("list-at out of range")), half_w, half_h)
		pb = r3d_project_clip((List.get(poly, I64.to_u64_wrap((i + 1))) ?? crash("list-at out of range")), half_w, half_h)
		(mem1, _hit) = (if Culling.cull_backface(p0.pv_sx, p0.pv_sy, pa.pv_sx, pa.pv_sy, pb.pv_sx, pb.pv_sy) { (mem, 0) } else { r3d_rasterize_tri_sh!(mem, st, p0, pa, pb, ctx) })
		r3d_raster_fan_sh!(mem1, st, poly, (i + 1), ctx, half_w, half_h)
	}) })
	...
	r3d_gouraud_fan! = |mem, st, poly, i, mat, lights, eye, ambient, g0, half_w, half_h| (if ((i + 1) >= U64.to_i64_wrap(List.len(poly))) { (mem, 0) } else { ({
		...
		r3d_gouraud_fan!(mem1, st, poly, (i + 1), mat, lights, eye, ambient, g0, half_w, half_h)
	}) })

	r3d_raster_fan_single! : Mem.Mem, Renderer3D.R3dTriState, List(Renderer3D.ClipVert), I64, Renderer3D.R3dShadeCtx, I64, I64 => (Mem.Mem, I64)
	r3d_raster_fan_single! = |mem, st, poly, i, ctx, half_w, half_h| ({
		p0 = r3d_project_clip((List.get(poly, I64.to_u64_wrap(0)) ?? crash("list-at out of range")), half_w, half_h)
		pa = r3d_project_clip((List.get(poly, I64.to_u64_wrap(i)) ?? crash("list-at out of range")), half_w, half_h)
		pb = r3d_project_clip((List.get(poly, I64.to_u64_wrap((i + 1))) ?? crash("list-at out of range")), half_w, half_h)
	...
	r3d_render_tris_phong! = |mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, i, n| (if (i >= n) { (mem, 0) } else { ({
		...
		c2 = r3d_clip_vertex(Mesh.mesh_vertex_at(mesh, i2), mvp, model)
		poly = r3d_clip_tri(c0, c1, c2)
		(if (U64.to_i64_wrap(List.len(poly)) < 3) { r3d_render_tris_phong!(mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, (i + 3), n) } else { ({
			ctx = r3d_shade_ctx_phong(c0, c1, c2, mat, lights, eye, ambient)
			(mem1, _hit) = r3d_raster_fan_sh!(mem, st, poly, 1, ctx, half_w, half_h)
			r3d_render_tris_phong!(mem1, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, (i + 3), n)
		}) })
	}) })
	...
	r3d_render_into! = |mem, st, scene| ({
		vp = Scene3D.camera3d_vp(scene.s3_camera)
		fr = Culling.frustum_extract(vp)
		r3d_render_nodes!(mem, scene, vp, fr, st, 0, scene.s3_count, st.r3t_w, st.r3t_h)
	})
	...
	r3d_render_nodes! = |mem, scene, vp, fr, st, i, n, w, h| (if (i >= n) { (mem, 0) } else { ({
		node = (List.get(scene.s3_nodes, I64.to_u64_wrap(i)) ?? crash("list-at out of range"))
		(mem1, _drawn) = (if node.sn3_visible { r3d_render_node!(mem, scene, vp, fr, st, node, i, w, h) } else { (mem, 0) })
		r3d_render_nodes!(mem1, scene, vp, fr, st, (i + 1), n, w, h)
	}) })

	r3d_render_node! : Mem.Mem, Scene3D.Scene3DState, Matrix4.Mat4, Culling.Frustum, Renderer3D.R3dTriState, Scene3D.SceneNode3D, I64, I64, I64 => (Mem.Mem, I64)
	r3d_render_node! = |mem, scene, vp, fr, st, node, idx, w, h| (match node.sn3_mesh {
		None => (mem, 0)
		Just(mesh) => ({
			model = Scene3D.scene3d_world_matrix(scene, idx)
			verdict = Culling.frustum_test_aabb(fr, r3d_world_bounds(Mesh.mesh_bounds(mesh), model))
	...
	r3d_render_mesh! : Mem.Mem, Renderer3D.R3dTriState, Mesh.Mesh, Matrix4.Mat4, Matrix4.Mat4, Material.EngineMaterial, List(Scene3D.Light3D), Quaternion.Vec3, I64, I64, I64 => (Mem.Mem, I64)
	r3d_render_mesh! = |mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h| ({
		shading = mat.emat_shading
		(match shading {
			Unlit => r3d_render_unlit_dispatch!(mem, st, mesh, mvp, mat, half_w, half_h)
			FlatShaded => r3d_render_tris_flat!(mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, 0, mesh.mesh_index_count)
			GouraudShaded => r3d_render_tris_gouraud!(mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, 0, mesh.mesh_index_count)
			PhongShaded => r3d_render_tris_phong!(mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, 0, mesh.mesh_index_count)
		})
	})

	r3d_render_unlit_dispatch! : Mem.Mem, Renderer3D.R3dTriState, Mesh.Mesh, Matrix4.Mat4, Material.EngineMaterial, I64, I64 => (Mem.Mem, I64)
	r3d_render_unlit_dispatch! = |mem, st, mesh, mvp, mat, half_w, half_h| (match mat.emat_texture {
		None => r3d_render_tris_unlit!(mem, st, mesh, mvp, mat, half_w, half_h, 0, mesh.mesh_index_count)
		Just(_t) => r3d_render_tris_unlit_tex!(mem, st, mesh, mvp, mat, half_w, half_h, 0, mesh.mesh_index_count)
	})
	...
	r3d_render_tris_unlit_tex! = |mem, st, mesh, mvp, mat, half_w, half_h, i, n| (if (i >= n) { (mem, 0) } else { ({
		...
		(mem1, _drawn) = r3d_raster_fan_sh!(mem, st, poly, 1, ctx, half_w, half_h)
		r3d_render_tris_unlit_tex!(mem1, st, mesh, mvp, mat, half_w, half_h, (i + 3), n)
	}) })
	...
	r3d_render_tris_flat! = |mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, i, n| (if (i >= n) { (mem, 0) } else { ({
		...
		(if (U64.to_i64_wrap(List.len(poly)) < 3) { r3d_render_tris_flat!(mem, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, (i + 3), n) } else { ({
			...
			(mem1, _drawn) = r3d_raster_fan_sh!(mem, st, poly, 1, ctx, half_w, half_h)
			r3d_render_tris_flat!(mem1, st, mesh, model, mvp, mat, lights, eye, ambient, half_w, half_h, (i + 3), n)
		}) })
	}) })
