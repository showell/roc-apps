...
GpuRender :: [].{
	...
	gr_tri! = |machine, idx, x0, y0, x1, y1, x2, y2, c0, c1, c2, depth| ({
		(machine19, machine__25) = ({
		off = (idx * 72)
		({
			(machine1, machine__7) = Machine.store!(machine, gr_cmd, off, x0, 4)
			(machine2, machine__8) = Machine.store!(machine1, gr_cmd, (off + 4), y0, 4)
			(machine3, machine__9) = Machine.store!(machine2, gr_cmd, (off + 8), x1, 4)
			(machine4, machine__10) = Machine.store!(machine3, gr_cmd, (off + 12), y1, 4)
			(machine5, machine__11) = Machine.store!(machine4, gr_cmd, (off + 16), x2, 4)
			(machine6, machine__12) = Machine.store!(machine5, gr_cmd, (off + 20), y2, 4)
			(machine7, machine__13) = Machine.store!(machine6, gr_cmd, (off + 24), c0, 4)
			(machine8, machine__14) = Machine.store!(machine7, gr_cmd, (off + 28), c1, 4)
			(machine9, machine__15) = Machine.store!(machine8, gr_cmd, (off + 32), c2, 4)
			(machine10, machine__16) = Machine.store!(machine9, gr_cmd, (off + 36), depth, 4)
			(machine11, machine__17) = Machine.store!(machine10, gr_cmd, (off + 40), depth, 4)
			(machine12, machine__18) = Machine.store!(machine11, gr_cmd, (off + 44), depth, 4)
			(machine13, machine__19) = Machine.store!(machine12, gr_cmd, (off + 48), 0, 4)
			(machine14, machine__20) = Machine.store!(machine13, gr_cmd, (off + 52), 0, 4)
			(machine15, machine__21) = Machine.store!(machine14, gr_cmd, (off + 56), 0, 4)
			(machine16, machine__22) = Machine.store!(machine15, gr_cmd, (off + 60), 0, 4)
			(machine17, machine__23) = Machine.store!(machine16, gr_cmd, (off + 64), 0, 4)
			(machine18, machine__24) = Machine.store!(machine17, gr_cmd, (off + 68), 0, 4)
			(machine18, (((((((((((((((((machine__7 + machine__8) + machine__9) + machine__10) + machine__11) + machine__12) + machine__13) + machine__14) + machine__15) + machine__16) + machine__17) + machine__18) + machine__19) + machine__20) + machine__21) + machine__22) + machine__23) + machine__24))
		})
	})
		(machine19, machine__25)
	})
	...
	gr_rect! = |machine, idx, x, y, w, h, color, depth| ({
		(machine1, machine__26) = gr_tri!(machine, idx, x, y, (x + w), y, x, (y + h), color, color, color, depth)
		(machine2, machine__27) = gr_tri!(machine1, (idx + 1), (x + w), y, (x + w), (y + h), x, (y + h), color, color, color, depth)
		(machine2, (machine__26 + machine__27))
	})
	...
	gr_render_tree! = |machine, gf, w, th, depth| ({
		(machine1, machine__28) = gr_render_node!(machine, gf, w, th, depth)
		gr_render_children!(machine1, machine__28, w.wn_children, th, depth, 0, w.wn_child_count)
	})
	...
	gr_render_children! = |machine, gf, children, th, depth, i, n| (if (i >= n) { (machine, gf) } else { ({
		(machine1, machine__29) = gr_render_tree!(machine, gf, (List.get(children, I64.to_u64_wrap(i)) ?? crash("list-at out of range")), th, (depth - 1))
		gr_render_children!(machine1, machine__29, children, th, depth, (i + 1), n)
	}) })
	...
	gr_render_button! = |machine, gf, w, th, txt, depth| ({
		...
		({
			(machine1, machine__30) = gr_emit_box!(machine, gf, br, style.ws_bg, style.ws_border, depth)
			gr_emit_text!(machine1, machine__30, txt, content.lr_x, content.lr_y, Theme.theme_tone_fg(th, w.wn_tone, style.ws_fg), text_depth, content.lr_w, TextOverflow.text_overflow_default)
		})
	})
	...
	gr_render_gauge! = |machine, gf, w, th, val, max_val, depth| ({
		...
		({
			(machine1, machine__31) = gr_emit_box!(machine, gf, br, style.ws_bg, style.ws_border, depth)
			gr_emit_rect!(machine1, machine__31, content.lr_x, content.lr_y, fill_w, content.lr_h, style.ws_fg, (depth - 1))
		})
	})
	...
	gr_render_input! = |machine, gf, w, th, txt, cursor, depth| ({
		...
		({
			(machine1, machine__32) = gr_emit_box!(machine, gf, br, style.ws_bg, style.ws_border, depth)
			(machine2, machine__33) = gr_emit_text!(machine1, machine__32, txt, content.lr_x, content.lr_y, style.ws_fg, text_depth, content.lr_w, TextOverflow.text_overflow_default)
			gr_emit_rect!(machine2, machine__33, cursor_x, content.lr_y, 1, 7, style.ws_fg, text_depth)
		})
	})
	...
	gr_render_overlay! = |machine, gf, ov, th| (if ov.ov_visible { ({
		...
		({
			(machine1, machine__34) = gr_emit_rect!(machine, gf, ov.ov_x, ov.ov_y, ov.ov_width, ov.ov_height, th.th_palette.pal_bg, gr_depth_overlay)
			gr_render_tree!(machine1, machine__34, laid, th, gr_depth_overlay)
		})
	}) } else { (machine, gf) })
	...
	gr_render_overlays! = |machine, gf, os, th, i, n| (if (i >= n) { (machine, gf) } else { ({
		(machine1, machine__35) = gr_render_overlay!(machine, gf, (List.get(os.os_overlays, I64.to_u64_wrap(i)) ?? crash("list-at out of range")), th)
		gr_render_overlays!(machine1, machine__35, os, th, (i + 1), n)
	}) })
	...
	gr_emit_rect! = |machine, gf, x, y, w, h, color, depth| ({
		(machine4, machine__39) = (if (w <= 0) { (machine, gf) } else { ({
		(machine3, machine__38) = (if (h <= 0) { (machine, gf) } else { ({
		(machine2, machine__37) = (if ((gf.gf_idx + 2) > gf.gf_max) { (machine, gf) } else { ({
		(machine1, machine__36) = gr_rect!(machine, gf.gf_idx, x, y, w, h, color, depth)
		(machine1, gr_frame_bump(machine__36, gf))
	}) })
		(machine2, machine__37)
	}) })
		(machine3, machine__38)
	}) })
		(machine4, machine__39)
	})
	...
	gr_emit_box! = |machine, gf, r, bg, bdr, depth| ({
		...
		({
			(machine1, machine__40) = gr_emit_rect!(machine, gf, (r.lr_x + wl), iy, iw, ih, bg, depth)
			(machine2, machine__41) = gr_emit_rect!(machine1, machine__40, r.lr_x, r.lr_y, r.lr_w, wt, bdr.bdr_top.brd_color, depth)
			(machine3, machine__42) = gr_emit_rect!(machine2, machine__41, r.lr_x, ((r.lr_y + r.lr_h) - wb), r.lr_w, wb, bdr.bdr_bottom.brd_color, depth)
			(machine4, machine__43) = gr_emit_rect!(machine3, machine__42, r.lr_x, iy, wl, ih, bdr.bdr_left.brd_color, depth)
			gr_emit_rect!(machine4, machine__43, ((r.lr_x + r.lr_w) - wr), iy, wr, ih, bdr.bdr_right.brd_color, depth)
		})
	})
	...
	gr_emit_text! = |machine, gf, txt, x, y, color, depth, avail, mode| ({
		...
		({
			(machine1, machine__44) = gr_emit_text_loop!(machine, gf, txt, x, y, color, depth, start, (start + count), start)
			gr_emit_dots!(machine1, machine__44, (x + (count * gr_glyph_advance)), y, color, depth, 0, dots)
		})
	})
	...
	gr_emit_text_loop! = |machine, gf, txt, x, y, color, depth, i, stop, start| (if (i >= stop) { (machine, gf) } else { ({
		(machine1, machine__45) = gr_emit_glyph!(machine, gf, gr_text_char_at(txt, i), (x + ((i - start) * gr_glyph_advance)), y, color, depth)
		gr_emit_text_loop!(machine1, machine__45, txt, x, y, color, depth, (i + 1), stop, start)
	}) })
	...
	gr_emit_dots! = |machine, gf, x, y, color, depth, k, n| (if (k >= n) { (machine, gf) } else { ({
		(machine1, machine__46) = gr_emit_glyph!(machine, gf, gr_text_char_at(".", 0), (x + (k * gr_glyph_advance)), y, color, depth)
		gr_emit_dots!(machine1, machine__46, x, y, color, depth, (k + 1), n)
	}) })
	...
	gr_emit_glyph_rows! = |machine, gf, rows, x, y, color, depth, row, max_row| (if (row >= max_row) { (machine, gf) } else { ({
		(machine1, machine__47) = gr_emit_glyph_cols!(machine, gf, (List.get(rows, I64.to_u64_wrap(row)) ?? crash("list-at out of range")), x, (y + row), color, depth, 0, 5)
		gr_emit_glyph_rows!(machine1, machine__47, rows, x, y, color, depth, (row + 1), max_row)
	}) })
	...
	gr_emit_glyph_cols! = |machine, gf, bits, x, y, color, depth, col, max_col| (if (col >= max_col) { (machine, gf) } else { (if (I64.bitwise_and(bits, I64.shl_wrap(1, I64.to_u8_wrap((4 - col)))) > 0) { ({
		(machine1, machine__48) = gr_emit_rect!(machine, gf, (x + col), y, 1, 1, color, depth)
		gr_emit_glyph_cols!(machine1, machine__48, bits, x, y, color, depth, (col + 1), max_col)
