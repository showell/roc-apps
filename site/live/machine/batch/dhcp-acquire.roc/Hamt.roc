...
Hamt :: [].{
	HamtEntry(a) : { key : Str, value : a }
	HamtNode(a) := [HamtEmpty, HamtLeaf(I64, Str, a), HamtCollision(I64, List(Hamt.HamtEntry(a))), HamtBranch(I64, List(Hamt.HamtNode(a)))].{
	...
	hamt_djb2_hash : Str -> I64
	hamt_djb2_hash = |s| hamt_djb2_loop(s, 0, Cce.length(s), 5381)

	hamt_djb2_loop : Str, I64, I64, I64 -> I64
	hamt_djb2_loop = |s, i, len, h| (if (i == len) { abs_int(h) } else { hamt_djb2_loop(s, (i + 1), len, I64.plus_wrap(I64.times_wrap(h, 33), Cce.at_or_crash(s, i))) })
	...
	hamt_get : Hamt.HamtMap(a), Str -> Maybe.Maybe(a)
	hamt_get = |m, key| ({
		hash = hamt_djb2_hash(key)
		hamt_node_get(m.root, hash, key, 0)
	})

	hamt_node_get : Hamt.HamtNode(a), I64, Str, I64 -> Maybe.Maybe(a)
	...
	collision_find : List(Hamt.HamtEntry(a)), Str -> Maybe.Maybe(a)
	...
	hamt_set : Hamt.HamtMap(a), Str, a -> Hamt.HamtMap(a)
	...
	hamt_node_set : Hamt.HamtNode(a), I64, Str, a, I64 -> Hamt.HamtSetResult(a)
	hamt_node_set = |node, hash, key, value, level| (match node {
		...
		HamtCollision(h, entries) => (if (h == hash) { ({
			...
			branch = hamt_node_set(make_single_branch(col_node, h, level), hash, key, value, level)
			branch
		}) })
		HamtBranch(bitmap, children) => ({
			chunk = extract_chunk(hash, level)
	...
	make_branch : I64, Str, a, I64, Str, a, I64 -> Hamt.HamtNode(a)
	...
	collision_set : List(Hamt.HamtEntry(a)), Str, a -> Hamt.CollisionSetResult(a)
	collision_set = |entries, key, value| collision_set_loop(entries, key, value, 0)

	collision_set_loop : List(Hamt.HamtEntry(a)), Str, a, I64 -> Hamt.CollisionSetResult(a)
	...
	hamt_remove : Hamt.HamtMap(a), Str -> Hamt.HamtMap(a)
	hamt_remove = |m, key| ({
		hash = hamt_djb2_hash(key)
	...
	hamt_node_remove : Hamt.HamtNode(a), I64, Str, I64 -> Maybe.Maybe(Hamt.HamtNode(a))
	...
	collision_remove : List(Hamt.HamtEntry(a)), Str -> List(Hamt.HamtEntry(a))
	collision_remove = |entries, key| collision_remove_acc(entries, key, [])

	collision_remove_acc : List(Hamt.HamtEntry(a)), Str, List(Hamt.HamtEntry(a)) -> List(Hamt.HamtEntry(a))
	...
	hamt_contains : Hamt.HamtMap(a), Str -> Bool
	...
	hamt_fold : (a, Str, b -> a), a, Hamt.HamtMap(b) -> a
	hamt_fold = |f, init, m| hamt_node_fold(f, init, m.root)

	hamt_node_fold : (a, Str, b -> a), a, Hamt.HamtNode(b) -> a
	...
	fold_entries : (a, Str, b -> a), a, List(Hamt.HamtEntry(b)), I64 -> a
	...
	fold_children : (a, Str, b -> a), a, List(Hamt.HamtNode(b)), I64 -> a
	...
	lam_0 : List(Hamt.HamtEntry(a)), Str, a -> List(Hamt.HamtEntry(a))
