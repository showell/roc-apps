...
# GpuPanelBorder -- emitted from Codex by rocemit (rust-codex-compiler). Do not edit.
import BoxModel
import GpuRender
import Machine
import Theme
...
probe_tree = Widget.widget_panel("root", DirColumn, 4, [Widget.widget_set_min(Widget.widget_panel("inner", DirColumn, 0, [Widget.widget_label("text", "panel")]), 0, 60)])

# --- Entry ---

main! = |args| {
	machine = Machine.boot!(args, ["Gpu", "Console"])
	(machine6, machine__47) = ({
		...
		({
			...
			({
				...
				(machine5, line!(Str.concat("panel-border: ", I64.to_str((if (machine__46 > 0) { 1 } else { 0 })))))
			})
		})
	})
	machine__47
	Machine.halt!(machine6)
	Ok({})
}
