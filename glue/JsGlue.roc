...
## **A STR IS THREE WORDS: bytes, capacity, length.** A small one keeps its
## bytes in the three words themselves and says so in the high bit of the last
## byte, whose low seven bits are then its length. The bytes are UTF-8.
str_reader : Str
str_reader =
	Str.concat(
