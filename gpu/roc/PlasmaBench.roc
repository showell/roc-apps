...
main! = |args| {
	n = I64.from_str(List.get(args, 0) ?? "10") ?? 10
	dev = frames(Device.new([List.repeat(0, I64.to_u64_wrap(w * h))]), 0, n - 1)
	out = Device.buffer(dev, 0)
	sum = List.fold(out, 0, |acc, v| U64.plus_wrap(acc, I64.to_u64_wrap(v)))
	echo!(Str.concat(Str.concat(Str.concat("frames ", I64.to_str(n)), " checksum "), Str.concat(U64.to_str(sum), "\n")))
	Ok({})
}
