...
MlpKernels :: [].{
	...
	kernel_matmul_relu : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	kernel_matmul : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	mlp_matmul_row : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	mlp_matmul_row = |dev, weights, input, bias, output, row, cols| ({
		(dev1, sum) = mlp_dot_row(dev, weights, input, bias, row, cols)
		Device.store(dev1, output, row, sum)
	})

	mlp_dot_row : Device.Device, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	mlp_dot_row = |dev, weights, input, bias, row, cols| ({
		(dev1, b) = Device.load(dev, bias, row)
		mlp_dot_accum(dev1, weights, input, row, cols, 0, b)
	})

	mlp_dot_accum : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	mlp_dot_accum = |dev, weights, input, row, cols, col, acc| (if (col >= cols) { (dev, acc) } else { ({
		(dev1, w) = Device.load(dev, weights, I32.plus_wrap(I32.times_wrap(row, cols), col))
		(dev2, x) = Device.load(dev1, input, col)
		mlp_dot_accum(dev2, weights, input, row, cols, I32.plus_wrap(col, 1), I32.plus_wrap(acc, I32.times_wrap(w, x)))
	}) })

	kernel_relu_grad : Device.Device, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	mlp_relu_grad_one : Device.Device, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	kernel_outer_product_add : Device.Device, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	mlp_outer_one : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	mlp_outer_one = |dev, grad_weights, delta, input, row, col, cols| ({
		(dev1, d) = Device.load(dev, delta, row)
		(dev2, x) = Device.load(dev1, input, col)
		(dev3, old) = Device.load(dev2, grad_weights, I32.plus_wrap(I32.times_wrap(row, cols), col))
		Device.store(dev3, grad_weights, I32.plus_wrap(I32.times_wrap(row, cols), col), I32.plus_wrap(old, I32.times_wrap(d, x)))
	})

	kernel_bias_grad_add : Device.Device, I32, I32, I32 -> (Device.Device, I32)
	kernel_bias_grad_add = |dev, grad_bias, delta, n| ({
		(dev1, tid) = Device.thread_idx_x(dev)
		(if (tid < n) { mlp_bias_one(dev1, grad_bias, delta, tid) } else { (dev1, 0) })
	})

	mlp_bias_one : Device.Device, I32, I32, I32 -> (Device.Device, I32)
	mlp_bias_one = |dev, grad_bias, delta, tid| ({
		(dev1, old) = Device.load(dev, grad_bias, tid)
		(dev2, d) = Device.load(dev1, delta, tid)
		Device.store(dev2, grad_bias, tid, I32.plus_wrap(old, d))
	})

	kernel_matmul_transpose : Device.Device, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	mlp_transpose_col : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	mlp_transpose_accum : Device.Device, I32, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	mlp_transpose_accum = |dev, weights, delta, col, rows, cols, row, acc| (if (row >= rows) { (dev, acc) } else { ({
		(dev1, w) = Device.load(dev, weights, I32.plus_wrap(I32.times_wrap(row, cols), col))
		(dev2, d) = Device.load(dev1, delta, row)
		mlp_transpose_accum(dev2, weights, delta, col, rows, cols, I32.plus_wrap(row, 1), I32.plus_wrap(acc, I32.times_wrap(w, d)))
	}) })

	kernel_adam_update : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	mlp_adam_one : Device.Device, I32, I32, I32, I32, I32, I32 -> (Device.Device, I32)
	...
	kernel_mse_grad : Device.Device, I32, I32, I32, I32 -> (Device.Device, I32)
	kernel_mse_grad = |dev, predicted, target, grad_out, n| ({
		(dev1, tid) = Device.thread_idx_x(dev)
		(if (tid < n) { mlp_mse_grad_one(dev1, predicted, target, grad_out, tid) } else { (dev1, 0) })
	})

	mlp_mse_grad_one : Device.Device, I32, I32, I32, I32 -> (Device.Device, I32)
	mlp_mse_grad_one = |dev, predicted, target, grad_out, tid| ({
		(dev1, p) = Device.load(dev, predicted, tid)
		(dev2, t) = Device.load(dev1, target, tid)
		Device.store(dev2, grad_out, tid, I32.times_wrap(2, I32.minus_wrap(p, t)))
	})
}
