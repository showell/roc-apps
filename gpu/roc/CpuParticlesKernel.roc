# Hand port of apps/gpushow/kernels/CpuParticlesKernel.codex (cobblestone-u58),
# in the shape rocemit is to write for an act under [Device]: each
# `x <- device-load` binds the value and the next device, each store the
# same, and the last statement's pair is the kernel's result.
import Device
