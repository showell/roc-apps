...
GearsKernel :: [].{
	...
	gr_gear : F64, F64, F64, F64, F64, F64, F64 -> F64
	gr_gear = |px, py, cx, cy, teeth, rtip, spin| ({
		...
		(if (r > rtip) { (0.0 - 1.0) } else { (if (r < (rtip * 0.22)) { (0.0 - 1.0) } else { ({
			...
			k = F64.to_i64_wrap((((a * teeth) / 3.1415926) + 2000.0))
			toothon = (k - (I64.div_trunc_by(k, 2) * 2))
			rroot = (rtip * 0.8)
	...
	gr_render : I64, I64 -> I64
	gr_render = |gid, frame| ({
		px = I64.to_f64((gid - (I64.div_trunc_by(gid, gr_width) * gr_width)))
		py = I64.to_f64(I64.div_trunc_by(gid, gr_width))
		f = (I64.to_f64(frame) / 30.0)
	...
	gr_pack : F64, F64, F64, F64 -> I64
	gr_pack = |lit, r, g, b| ({
		ri = F64.to_i64_wrap(DeviceMath.real_min(255.0, ((lit * r) * 255.0)))
		gi = F64.to_i64_wrap(DeviceMath.real_min(255.0, ((lit * g) * 255.0)))
		bi = F64.to_i64_wrap(DeviceMath.real_min(255.0, ((lit * b) * 255.0)))
		(((ri * 65536) + (gi * 256)) + bi)
	})

	gears_step : Device.Device, I64, I64, I64 -> (Device.Device, I64)
