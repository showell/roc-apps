...
CatDraw :: [].{
	...
	cat_pts = |b, h, lift, ps, i| (if (i >= U64.to_i64_wrap(List.len(ps))) { [] } else { List.concat([cat_pt(b, h, lift, (List.get(ps, I64.to_u64_wrap(i)) ?? crash("list-at out of range")))], cat_pts(b, h, lift, ps, (i + 1))) })
	...
	cat_polys = |b, h, lift, ps, i| (if (i >= U64.to_i64_wrap(List.len(ps))) { [] } else { List.concat(cat_poly(b, h, lift, (List.get(ps, I64.to_u64_wrap(i)) ?? crash("list-at out of range"))), cat_polys(b, h, lift, ps, (i + 1))) })
