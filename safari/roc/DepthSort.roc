...
DepthSort :: [].{
	...
	rest_from_acc = |ys, j, acc| (if (j >= U64.to_i64_wrap(List.len(ys))) { acc } else { rest_from_acc(ys, (j + 1), List.concat(acc, [(List.get(ys, I64.to_u64_wrap(j)) ?? crash("list-at out of range"))])) })
