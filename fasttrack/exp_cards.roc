...
main! = |_args| {
	...
	Echo.line!("Drawn is played + discarded + in hand at the end. Ranked by the winner's share of a card's draws; over every card it is ${Arena.thousandths(I64.to_f64(all_w) / I64.to_f64(all_w + all_l))} (winners draw more cards), and the ± is one standard error, counting draws as independent.\n")
	...
	for r in List.map_with_index(ranked, |r, i| { card: r.card, score: r.score, w: r.w, l: r.l, wd: r.wd, ld: r.ld, wh: r.wh, lh: r.lh, dw: r.dw, dl: r.dl, share: r.share, se: r.se, rank: i + 1 }) {
		Echo.line!("| ${U64.to_str(r.rank)} | ${r.card} | ${Arena.thousandths(r.share)} ± ${Arena.thousandths(r.se)} | ${I64.to_str(r.dw)} / ${I64.to_str(r.dl)} | ${I64.to_str(r.w)} / ${I64.to_str(r.l)} | ${I64.to_str(r.wd)} / ${I64.to_str(r.ld)} | ${I64.to_str(r.wh)} / ${I64.to_str(r.lh)} | ${I64.to_str(r.score)} |")
	}
