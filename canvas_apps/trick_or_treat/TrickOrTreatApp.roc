# TrickOrTreatApp -- the Halloween movie as a value canvas_apps can run.
#
# **A MOVIE IS A GAME THAT DOES NOT READ ITS KEYBOARD.** That is the whole of
# what this file had to work out. `CanvasApp` and `Movie` asked for the same size,
...
TrickOrTreatApp :: [].{
	...
	canvas_app : CanvasApp.CanvasApp(TrickOrTreatApp.Model)
	canvas_app = {
	...
	step : TrickOrTreatApp.Model, Input.Snapshot -> TrickOrTreatApp.Model
	step = |m, input| {
...
expect {
    held = TrickOrTreatApp.step(TrickOrTreatApp.canvas_app.init, Input.none.with_key_pressed(KeySpace))
    held.paused and TrickOrTreatApp.step(held, Input.none).tick == held.tick
}
