...
## `CanvasApp` is this vocabulary's own type. Its import line is the only other change:
...
apply_controls : Rules.World, Rules.Controls -> Rules.World
...
step_snake : Rules.World -> (Rules.World, List(Rules.Event))
step_snake = |world| {
...
advance_fixed_steps : Rules.World, List(Rules.Event) -> (Rules.World, List(Rules.Event))
...
update_game_over : Rules.World, Rules.Controls -> (Rules.World, List(Rules.Event))
...
expect {
	world = { ..Rules.new_world(Random.seed(1)), food: { x: 13, y: 9 } }
	(next, events) = step_snake(world)
...
expect {
	world = Rules.new_world(Random.seed(1))
	(next, _events) = Rules.update(world, no_controls, step_time * 2)
