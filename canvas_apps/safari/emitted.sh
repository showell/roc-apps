...
#
#   canvas_apps/safari/emitted.sh             all 54 units; rewrites canvas_apps/safari/ whole
#   canvas_apps/safari/emitted.sh ViewYaw     one; rewrites only what that unit emits
#   JOBS=1 canvas_apps/safari/emitted.sh      serial (the default runs two units at a time)
#
...
#
# THE ROC IS TRACKED. `canvas_apps/safari/` is generated and committed -- the one
...
#
# canvas_apps/safari/*.roc is SOURCE now, hand-edited like any other Roc. This script
...
echo "retired: canvas_apps/safari is the program now, not an emission. See the comment in $0." >&2
exit 2
...
units=()
# PHASE ONE, serial: emit every unit, check chapter identity, fill canvas_apps/safari.
