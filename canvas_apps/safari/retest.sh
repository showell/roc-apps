...
# tracked canvas_apps/safari/, and runs, through emitted.sh, exactly the specs whose
# app or imported modules differ. Nothing changed means nothing runs, and
# says so. A full sweep is still the gate before a commit of canvas_apps/safari.
set -u
...
if [ ${#to_run[@]} -eq 0 ]; then echo "nothing changed against canvas_apps/safari; nothing to run"; exit 0; fi
