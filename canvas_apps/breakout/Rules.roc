...
##
## Ported from roc-ray's examples/breakout, where it is Game.roc; renamed
## because `CanvasApp` is this vocabulary's own type. Its import line is the rest of the
## diff: the rules never touched the platform.
import lib.Math
import Ball
import Bricks
import Paddle
...
event_when : Bool, Rules.Event -> List(Rules.Event)
...
update_finished : Rules.World, Rules.Controls -> (Rules.World, List(Rules.Event))
update_finished = |world, controls| if controls.action_pressed (Rules.new_world(), [GameStarted]) else (world, [])
