...
| `roc/MachineApic.roc` | the boot core's local APIC (id, version, SIVR, the command register, the timer's count on the machine's clock) and the IOAPIC's redirection table; neither delivers an interrupt, and the run stops where one would |
...
| `roc/MachineHpet.roc` | the HPET at 0xFED00000, counting the machine's own clock, which every device register access moves 10 µs (`Machine.access_cost`); timer 0's comparator, checked whenever the program touches the HPET |
...
| `roc/MachineGpu.roc` | the GPU codex-vm models behind ports 0x400-0x417: the command buffer, depth buffer and framebuffer as one flat plane a pixel writes in place, clear (0x401), depth clear (0x402) and flush (0x400: codex-vm's triangle rasterizer, then its glow); a padded stride draws nothing, and the ports and triangles no program here uses stop by name. `MachineGpuTests.roc` is `roc test`'s |
| `roc/MachineScreen.roc` | the screen: `Machine.halt!`, which every emitted program ends with, hands a platform that shows the screen the framebuffer at 0xBF000000 that codex-vm's `-gop` flags describe; here nothing is shown and the framebuffer is not read, and `batch/MachineScreen.roc` hands it to the page |
